...
class ProductRepository extends BaseRepository_1.BaseRepository {
    ...
    sell(productId, salesData) {
        ...
        return this.update(productId, {
            ...
            deliveryDate: (_b = salesData.deliveryDate) !== null && _b !== void 0 ? _b : undefined,
...
function sellProduct(productId, salesData) {
    try {
        ...
        const convertedData = {
            ...salesData,
            soldDate: salesData.soldDate ? new Date(salesData.soldDate) : salesData.soldDate,
            soldPrice: Number(salesData.soldPrice),
            deliveryDate: salesData.deliveryDate ? new Date(salesData.deliveryDate) : undefined,
        };
...
function getMajorCategories() {
    try {
        const repo = new MasterRepository_1.MajorCategoryRepository(getSpreadsheetId());
        const result = repo.findAllSorted();
        return JSON.parse(JSON.stringify(result));
    }
    catch (error) {
...
// ==================== 大分類リポジトリ ====================
class MajorCategoryRepository extends BaseRepository_1.BaseRepository {
    constructor(spreadsheetId) {
        const config = {
            spreadsheetId,
            sheetName: common_1.SHEET_NAMES.MAJOR_CATEGORIES_MASTER,
            headers: master_1.MAJOR_CATEGORY_HEADERS,
        };
        super(config);
    }
    rowToEntity(row) {
        return (0, master_1.rowToMajorCategory)(row);
    }
    entityToRow(entity) {
        return (0, master_1.majorCategoryToRow)(entity);
    }
    getIdColumnIndex() {
        return 0; // categoryId
    }
...
// ==================== 大分類マスター ====================
exports.MAJOR_CATEGORY_COLUMNS = {
    CATEGORY_ID: 0,
    NAME: 1,
    DISPLAY_ORDER: 2,
};
exports.MAJOR_CATEGORY_HEADERS = ['カテゴリID', 'カテゴリ名', '表示順'];
function rowToMajorCategory(row) {
    var _a, _b;
